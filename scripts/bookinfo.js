...
function createBookInfo(bookdata) {
    ...
    bookdata.forEach((book, index) => {
        var bookInfoArea = $('<div/>').addClass('book-area').addClass('carousel-item');
        ...
        var bookFrontpage = $('<div/>').addClass('book-frontpage').appendTo(bookInfoArea);
...
function getgiffy(etag, bookInfoArea) {
    $.get(`https://api.giphy.com/v1/gifs/random?tag=${etag}.data&api_key=kJUvzKYBeb8ygT26srrOdvDd4BJ4ITYv`)
...
function getBookInfo(event) {
    ...
    $.get(`https://www.googleapis.com/books/v1/volumes?${q}&orderBy=newest&key=${apiKey}`)
